...
export default function AddProductPage() {
  ...
  const [imageColor, setImageColor] = useState('bg-slate-300');
  const [imageUrl, setImageUrl] = useState<string | undefined>();

  useEffect(() => {
    const colors = ['bg-rose-400', 'bg-blue-400', 'bg-emerald-400', 'bg-amber-400', 'bg-purple-400', 'bg-cyan-400'];
    setImageColor(colors[Math.floor(Math.random() * colors.length)]);
  }, []);
  ...
  return (
    <div className="p-6 md:p-8 max-w-3xl mx-auto h-full flex flex-col animate-in fade-in slide-in-from-bottom-4 duration-300">
      <div className="flex items-center mb-8 space-x-4">
        ...
        <div>
          <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight">เพิ่มสินค้าใหม่</h1>
          <p className="text-slate-500 font-medium mt-1">เพิ่มรายการสินค้าเข้าสู่ระบบคลังและแคตตาล็อก</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-3xl shadow-xl shadow-slate-200/40 border border-gray-100 flex-1 overflow-y-auto">
        <form onSubmit={handleSave} className="p-8 space-y-8">
            ...
            <div className="col-span-2 sm:col-span-1">
              ...
              <div className="flex items-center space-x-4">
                ...
                <label className="flex-1 p-4 border-2 border-dashed border-gray-300 rounded-xl text-center font-medium text-slate-500 hover:border-blue-500 hover:text-blue-600 cursor-pointer transition-colors">
                  <div className="text-sm">คลิกเพื่อเลือกไฟล์รูปภาพ</div>
                  <input type="file" accept="image/*" onChange={handleImageUpload} className="hidden" />
                </label>
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 gap-6 pt-6 border-t border-gray-100">
            <div className="col-span-2 sm:col-span-1">
              <label className="block text-slate-700 font-bold mb-2">สต็อกเริ่มต้น</label>
              <input 
                type="number" 
                value={stock}
